...
const RelatedProd = () => {
  ...
  return (
    <div className="horizontal-scroll-container">
      <button className="nav" onClick={scrollLeft}>&lt;</button>
            ...
            {productsData.map(({id,image,name,price,discount,score}) => (
              <div key={id} className="related-item">
                ...
                <div className="related-content">
                  <div><span className="related-price">${price}</span> <span className="related-discount">-{discount}% <img src={'/img/sale.png'} alt="Icon" /></span></div>
                  <div className="related-score">
                    {Array.from({ length: Stars }).map((_, index) => (
                      <span key={index} className={`icon-star ${index < score ? 'icon-star-full' : 'icon-star-empty'}`}></span>
                    ))}
                  </div>
      ...
      <button className="nav" onClick={scrollRight}>&gt;</button>
    </div>
  );
};
